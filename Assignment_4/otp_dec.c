...
char* ReadFileContent(FileInfoObject* Obj, char fd)
{
...
int main(int argc, char* argv[])
{
	...
	char* KeyText = ReadFileContent(FileInfo,'K');
	char* RecievedText = malloc( FileInfo->FileLength * sizeof(char*));
	char* PlainText= ReadFileContent(FileInfo,'T');
	char serverAccept;
	char fileLength[128];
	...
	send(socketFD,&(PROGNAME[0]), sizeof(char),0);//send client info D for decrption client.
	...
	send(socketFD,PlainText,FileInfo->FileLength * sizeof(char),0); // sent message text.

	//recieving data from server.
	recv(socketFD,RecievedText,FileInfo->FileLength * sizeof(char),0);//retrive decrypted text.
	printf("%s\n",RecievedText);
